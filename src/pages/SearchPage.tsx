...
export function SearchPage() {
  ...
  return (
    <>
      {/*
        Сырой текст ошибки прячем в подсказку: редакции он ничего не говорит, а
        пугает. Про сохранение молчим намеренно — запись идёт отдельным
        запросом, и её результат виден по уведомлению, а не по этому баннеру.
      */}
      {error && (
        <p className="banner banner--warn" title={error}>
          Показаны сохранённые данные — обновить базу не удалось. Свежих правок коллег в списке
          может не быть.{' '}
          <button className="btn btn--small btn--ghost" onClick={() => void reload()}>
            Обновить
          </button>
        </p>
      )}
