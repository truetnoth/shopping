...
// Регистр не плодит вариантов: «Сумки» из старых данных — та же кнопка, что
// «сумки» из справочника, и побеждает написание справочника.
check('другой регистр не даёт второй кнопки', optionsWithOwn(fields[2], 'Сумки'), ['одежда', 'верхняя одежда', 'сумки', 'обувь'])
...
// Старая текстовая колонка успела набрать «Не тестируется на животных» рядом с
// «не тестируется...» из справочника — две кнопки на одно и то же. Сливаем.
check(
  'пометка в другом регистре не двоится',
  optionsWithOwn(marks, '', [{ id: 'm4', 'Пометки': 'Не тестируется на животных' }]),
  ['не тестируется на животных'],
)
// Когда справочник не подсказывает канонического написания, остаётся то, что
// идёт первым по алфавиту, — важно лишь, что кнопка одна, а не две.
check(
  'дубль по регистру внутри данных схлопывается',
  optionsWithOwn(f('Город', { type: 'openselect' }), '', [
    { id: 'c1', 'Город': 'Казань' },
    { id: 'c2', 'Город': 'казань' },
  ]),
  ['казань'],
...
check(
  'написание из справочника важнее написания из данных',
  optionsWithOwn(f('Город', { type: 'openselect', options: ['Казань'] }), '', [
    { id: 'c1', 'Город': 'казань' },
  ]),
  ['Казань'],
)
